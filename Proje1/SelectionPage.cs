...
namespace Proje1
{
    public partial class SelectionPage : Form
    {
        ...
        private void btnBuy_Click(object sender, EventArgs e)
        {
                ...
                using (var session = NhibernateHelper.OpenSession())
                {
                    using (var transaction = session.BeginTransaction())
                    {
                        ...
                        var salesclass = new Model.Sales()
                        {
                            ...
                            adet = chairs.Count(),
                            seans = selectedSession.date + selectedSession.time,
                            musteri = veri1,
                            koltuk =a
            ...
            {
                MessageBox.Show("Lütfen bir isim giriniz.","Sistem Mesajı", MessageBoxButtons.OK);
            ...
            if (chairs.Count == 0)
            {
                MessageBox.Show("En az 1 (bir) adet koltuk seçmek zorundasınız.", "Sistem Mesajı", MessageBoxButtons.OK);
                return;
            }
            Sales sales = new Sales();
            Movie moviee = new Movie();
            moviee.movieName = selectedMovie.movieName;
            sales.adet = chairs.Count;
            sales.seans = $"{selectedSession.date} - {selectedSession.time}";
            sales.fiyat = CalculatePrice();
